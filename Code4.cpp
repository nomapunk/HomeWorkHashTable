...
public:
    ...
    HashMap(Hash hasher = Hash()): get_hash(hasher) {
        ...
        _begin = _end = iterator(indexes.begin(), array);
        _cbegin = _cend = const_iterator(_begin);
    }

    HashMap(HashMap& other): get_hash(other.get_hash) {
        buffer_size = default_size;
        _size = 0;
        size_all_non_nullptr = 0;
        array.resize(buffer_size);
    ...
    HashMap(std::initializer_list<std::pair<const KeyType, ValueType>> cur, Hash hasher = Hash()): get_hash(hasher) {
...
private:
    ...
    bool Remove(const KeyType& key) {
        for (size_t i = get_hash(key) % buffer_size, cnt = 0; cnt < buffer_size && array[i].second; cnt++) {
            if (array[i].first.entry.first == key && array[i].first.flag == true) {
                ...
                indexes.erase(array[i].first.it.it);
                _size--;
                return true;
            }
            if (i + 1 == array.size()) {
    ...
    int Add(const KeyType key, ValueType val) {
        ...
        if (already_exists) {
            ...
            array[i].first.it = iterator(indexes.insert(indexes.end(), i), array);
            ...
            array[i].second = true;
            array[i].first = Node(key, val, iterator(indexes.insert(indexes.end(), i), array));
            array[i].first.const_iterator = const_iterator(array[i].first.it);
            _end = array[i].first.it;
            _cend = array[i].first.const_iterator;
        }
